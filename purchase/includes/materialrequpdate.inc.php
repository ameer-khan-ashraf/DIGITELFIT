...
if(isset($_POST['update-mr']))
{
    ...
    $mstatus=$row['mstatus'];
    $remarks=$row['remarks'];
    $query= "UPDATE purchaseint SET pstatus=? WHERE ID='$id'";
    ...
    if(!mysqli_stmt_prepare($query_run,$query))
        ...
        {
            mysqli_stmt_bind_param($query_run, "s" ,$pstatus);
            mysqli_stmt_execute($query_run);
            $log= $_SESSION['username'].' Updated Material Request PO status at '.date("h:i:s").PHP_EOL.
           'Material Request: '.$id.' department: '.$dept.' Upload date '.$udate.' Expected date '.$date.' Product code '.$pcode.' Product description '.$pdesc.'PO Status '.$pstatus.' Status '.$mstatus.' Remarks '.$remarks.PHP_EOL.
            '-----------------------------------------------------------------------'.PHP_EOL;
            fwrite($myfile, $log);
...
elseif(isset($_POST['update-mrdate']))
{
    $id = $_GET['id'];
    $date = $_POST['ddate'];
    $pstatus = $_POST['pstatus'];
